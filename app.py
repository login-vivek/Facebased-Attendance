...
def assign_roll_numbers_api(classroom_id):
    ...
    for assignment in assignments:
        ...
        if not face_id or not roll_number:
            ...
            continue
        temp_face_path = os.path.join(temp_faces_dir, f"{face_id}.jpg")
        if os.path.exists(temp_face_path):
            new_face_path = os.path.join(labeled_faces_dir, f"{roll_number}.jpg")
            try:
...
def train_model_api(classroom_id):
    ...
    try:
        ...
        augment_faces(face_data_for_augmentation, classroom_id)
