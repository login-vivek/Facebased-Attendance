...
def delete_person(employee_id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": f"{person_name} removed from the system", "employee_id": employee_id})
...
def attendance():
    ...
    cursor      = conn.cursor()
    if date_filter:
        cursor.execute("SELECT * FROM attendance_log WHERE date = ? ORDER BY check_in DESC", (date_filter,))
    else:
        today = datetime.today().date().isoformat()
        cursor.execute("SELECT * FROM attendance_log WHERE date = ? ORDER BY check_in DESC", (today,))
    rows = cursor.fetchall()
    conn.close()
    result = []
    for row in rows:
        result.append({
            "id": row[0], "person_id": row[1], "employee_id": row[2],
            "name": row[3], "check_in": row[4], "date": row[5],
            "status": row[6], "confidence": row[7]
        })
    return jsonify(result)
...
def stats():
    ...
    cursor.execute("SELECT COUNT(*) FROM attendance_log WHERE date = ? AND status = 'Present'", (today,))
    present = cursor.fetchone()[0]
    cursor.execute("SELECT COUNT(*) FROM attendance_log WHERE date = ? AND status = 'Late'", (today,))
    late = cursor.fetchone()[0]
    conn.close()
    return jsonify({
        "total_enrolled": total, "present_today": present,
        "late_today": late, "absent_today": max(0, total - present - late), "date": today
    })
...
def enroll():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("INSERT INTO persons (name, employee_id, department, role) VALUES (?, ?, ?, ?)",
    ...
    for i, photo_b64 in enumerate(photos):
        try:
            ...
            cursor.execute("INSERT INTO face_photos (person_id, employee_id, photo_path) VALUES (?, ?, ?)",
    ...
    conn.commit()
    conn.close()
    return jsonify({"success": True, "message": f"{name} enrolled successfully with {saved} photo(s)!", "person_id": person_id})
...
def recognize():
    ...
    for person_id, name, employee_id, photo_path in enrolled:
        ...
        try:
            result   = DeepFace.verify(img1_path=temp_path, img2_path=photo_path, model_name="VGG-Face", enforce_detection=False)
            distance = result["distance"]
            if distance < best_distance:
                best_distance = distance
                best_match    = (person_id, name, employee_id)
        except Exception as e:
            print(f"[RECOGNIZE] Error: {e}"); continue
    if os.path.exists(temp_path): os.remove(temp_path)
    if best_match and best_distance < THRESHOLD:
        person_id, name, employee_id = best_match
        confidence = round((1 - best_distance) * 100, 1)
        status = override_status if override_status in ("Present", "Late") else ("Late" if datetime.now().hour >= 9 else "Present")
        conn   = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        today  = datetime.today().date().isoformat()
        now    = datetime.now().strftime("%H:%M:%S")
        cursor.execute("SELECT id FROM attendance_log WHERE employee_id = ? AND date = ?", (employee_id, today))
        existing = cursor.fetchone()
        if not existing:
            cursor.execute("INSERT INTO attendance_log (person_id, employee_id, name, date, check_in, status, confidence) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ...
        conn.close()
        return jsonify({"recognized": True, "name": name, "employee_id": employee_id, "confidence": confidence, "status": status, "attendance_logged": attendance_logged})
